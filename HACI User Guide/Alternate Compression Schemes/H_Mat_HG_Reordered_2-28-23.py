...
class CMatrix():
    ...
    def __init__(self, mat,type,level):
        """
        CMAtrix constructor.
...
#        print("k = ",k_svd)
        ...
        #H-mat first block
            ...
            j = self.nc // 2
            print(self.level)
            self.b11 = CMatrix(mat[:i,:j],2,self.level+2)
            self.b12 = CMatrix(mat[:i,j:],3,self.level)
            self.b21 = CMatrix(mat[i:,:j],3,self.level)
            self.b22 = CMatrix(mat[i:,j:],3,self.level)
...
if __name__ == "__main__":
    ...
    #print(newmat)
    #print('CMatrix:')
    #CMatrix(input_mat, split_type, start_rank)
    cmat= CMatrix(mat,2,2)
